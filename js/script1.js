...
$(document).ready(function() {
    ...
    var newCurrentDateTime = moment().add('days', 10);
    var hiddenVal = newCurrentDateTime;

    var callbackRangeCalendar = $("#dateRange1").rangeCalendar1({
        theme: "default-theme",
        changeRangeCallback: rangeChanged,
        start: "0",
        startRangeWidth: "1",
        startDate: currentDateTime,
        endDate: newCurrentDateTime
    });

    $('#dateRangeMonthPicker1').on('change', function() {
        var dateMonthTemp = parseInt($(this).val());
        var dateYearTemp = new Date().getFullYear();

        var newDateRangeStart = new Date(dateYearTemp, dateMonthTemp, 1);
        var newDateRangeEnd = moment(newDateRangeStart).add(10, 'days');
        ...
        hiddenVal = newDateRangeEnd;

        // Redefine Range when new month is selected
        currentDateTime = moment(hiddenVal).subtract('days', 10);
        newCurrentDateTime = hiddenVal;
    });

    $("#buttonDateNextWrap1").click(function() {
        var newDateVal = moment(hiddenVal).add(10, 'days');
    ...
    $("#buttonDatePreviousWrap1").click(function() {
        var startDateTemp = moment(hiddenVal).subtract(20, 'days');
        var endDateTemp = moment(hiddenVal).subtract(10, 'days');
    ...
    $('#dateRange1').on('click', '.cal-cell1[date-id1]', function() {
        var selectedDate1 = $(this).attr('date-id1');
        var startDay = parseInt(selectedDate1.slice(6, selectedDate1.length));
        var startMonth = parseInt(selectedDate1.slice(4, 6));
        var startYear = parseInt(selectedDate1.slice(0, 4));

        selectedDate1 = new Date(startYear, startMonth - 1, startDay);
    ...
    $('#dateRangeMonthPicker1 option:eq(' + currentMonth + ')').prop('selected', true);
...
(function($, window, undefined) {
    $.fn.rangeCalendar1 = function(options) {
        var defaults = { lang: "en", theme: "default-theme", themeContext: this, startDate: moment(), endDate: moment().add('months', 12), start: "+7", startRangeWidth: 3, minRangeWidth: 1, maxRangeWidth: 14, weekends: !0, autoHideMonths: !1, visible: !0, trigger: null, changeRangeCallback: function(el, cont, dateProp) { return !1 } };
        var returnObj;
        this.each(function(i, el) {
            ...
            obj.showCalendar = function(animate) {
                ...
                if (obj.visible) obj.showCalendar();
                else obj.hideCalendar()
                ...
                var cellWidth = obj.calendarObj.find(".cal-cell1").eq(0).outerWidth();
            ...
            }, obj.range = function() { var startDateIndex = obj.calendarObj.find('.cell.selected:eq(0)').index(); var endDateIndex = obj.calendarObj.find('.cell.selected').last().index(); var startDate = moment().add('days', startDateIndex + obj.start); var startDateFormatted = (startDateIndex >= 0 ? moment().add('days', startDateIndex + obj.start).format() : null); var endDateFormatted = (endDateIndex >= 0 ? moment().add('days', endDateIndex + obj.start).format() : null); var range = $.data(obj, "range", { start: startDateFormatted, end: endDateFormatted, width: obj.rangeWidth(), fromNow: startDate.fromNow() }); return range }, obj.setStartDate = function(startDate) {
                ...
                var dateCell = obj.calendarObj.find('.cell[date-id1="' + dateId + '"]').eq(0);
                dateCell.trigger("click");
                var monthCell = obj.monthsObj.find('.cell[month-id="' + monthId + '"]').eq(0);
                monthCell.trigger("click")
            }, obj.lang = function() { return obj.lang }, obj.setTheme = function(themeName) {
                var _themeName = $(obj.themeContext).attr("theme");
                if (_themeName) $(obj.themeContext).removeClass(_themeName);
                ...
                if (obj.isDragging || $(obj.lastTarget).is(obj.monthsObj)) { delete obj.lastTarget; return }
                ...
                if (obj.isDragging || $(obj.lastTarget).is(obj.calendarObj)) { delete obj.lastTarget; return }
                ...
                if (rightBar > lastCalItem.index()) { obj.calendarObj.find(' .cell').eq(currentCalItem.index() - rangeWidth + delta + 1).trigger("click"); return }
                ...
                if (obj.maxRangeWidth > 1) { obj.calendarObj.find(".range-bar").resizable({ grid: [cellWidth, 0], maxWidth: obj.maxRangeWidth * cellWidth, minWidth: cellWidth * obj.minRangeWidth, maxHeight: cellHeight, minHeight: cellHeight, handles: "e" }) }
                ...
                var x1, x2, y1, y2, t1, t2, minDistance = 40,
                    friction = 10;
                var onMouseMove = function(e) { var mouseEvents = draggableObj.data("mouseEvents"); if (e.timeStamp - mouseEvents[mouseEvents.length - 1].timeStamp > 40) { mouseEvents.push(e); if (mouseEvents.length > 2) { mouseEvents.shift() } } };
                ...
                draggableObj.bind('dragstop', function(e) {
                    ...
                    if (distance > minDistance) {
                        ...
                        var maxLeft = $(window).width() - (draggableObj.width() + 10),
                            maxTop = 0;
                        draggableObj.animate({ textIndent: 0 }, {
                            ...
                            step: function(currentStep) {
                                ...
                                draggableObj.css({ left: newLeft + "px", top: 0 + "px" })
                            }
                        })
                    }
                })
                ...
                var rangeWidth = obj.rangeWidth();
                for (var index = 1;
                    (date.isBefore(endDate) || date.isSame(endDate)); index++) {
                    var fullYear = date.format("YYYY");
                    ...
                    cell = '<div class="cal-cell1 cell" date-id1="' + fullYear + monthNumber + dayNumber + '" month-id="' + fullYear + '' + monthNumber + '" month="' + monthNumber + '">';
                ...
                callback(el, options)
            }, obj._placeElement = function(el) { obj._placeElement(el, null) }, obj._placeElement = function(el, position) {
                var calendarViewWidth = $el.outerWidth();
                ...
                if (calendarViewWidth > objWidth) left = (calendarViewWidth - objWidth) / 2;
                else if (calendarViewWidth < objWidth && left >= 0) left = 0;
                else if (left < calendarViewWidth - objWidth) left = -objWidth + calendarViewWidth;
                ...
                $el.hover(function() { if (obj.autoHideMonths) obj.monthsObj.slideDown(100, 'easeOutCirc') }, function() { if (obj.autoHideMonths) obj.monthsObj.slideUp(0, 'easeOutCirc') });
                $(obj).on('mousedown', '.range-bar', function(e) { var topElement = document.elementFromPoint(e.clientX, e.clientY); if ($(topElement).hasClass('ui-resizable-handle')) { e.stopPropagation(); return !1 } });
                $(obj).on('mouseup', '.range-bar', function(e) {
                    if ($(obj).find('.range-bar').hasClass('ui-resizable-resizing')) return !0;
                    $(this).hide();
            ...
            $(obj).data('rangeCalendar1', obj)
        });
        return this.data("rangeCalendar1")
    }
})(jQuery, window)