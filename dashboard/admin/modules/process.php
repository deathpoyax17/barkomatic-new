...
//*edit assigned id
function assign_edit_id($c) {
    if(session_start()) {
        $edit_id = $_POST['edit_role_id'];
        // $_SESSION['edit_role_id'] = $edit_id;
     
        $sql_updt = "UPDATE tbl_ship_detail tbl_sd 
                    INNER JOIN tbl_ship_account tbl_sa ON tbl_sd.id=tbl_sa.id
                    SET tbl_sd.stat=1 WHERE tbl_sd.id=?";
            ...
            if($stmt){
               ...
               $sql_slct = "SELECT 
                tbl_psrd.id,
                tbl_psrd.subscription_id,
                tbl_psrd.ship_name,
                tbl_psrd.email,
                tbl_psra.username,
                tbl_psra.password
                FROM tbl_ship_detail tbl_psrd
                INNER JOIN tbl_ship_account tbl_psra 
                ON tbl_psrd.id = ?";
     $stmt = $c->prepare($sql_slct);
     $stmt->bind_param('s',$edit_id);
     $stmt->execute();
     ...
     $row = $result->fetch_assoc();       
            try {
                ...
                $mail->Port = 25;
                $mail->setFrom('barkomatic@barkomatic.xyz', 'Application Status');
                $mail->addAddress($row['email']);
                $mail->isHTML(true);
                ...
                <body>
                    <div class='container m-auto'>
                        <div class='row m-auto'>
                            <div class='col-sm-12'>
                                <p>Congratulations,</p>
                                <p>This is to inform you that your Application is Approved as Shipping Line Owner.</p>
                            </div>
                        </div>
                    </div>
                </body>
...
function delete_assigned_role($c) {
    ...
    $sql_dlt = "DELETE tbl_ship_account,tbl_ship_detail FROM tbl_ship_detail 
                INNER JOIN tbl_ship_account ON tbl_ship_detail.id = tbl_ship_account.id
                WHERE tbl_ship_detail.id=?";

    $stmt = $c->prepare($sql_dlt);
    $stmt->bind_param('s',$delete_id);
    $stmt->execute();
    $stmt->close();
    echo "Disapproved Account Well be Deleted!";
}
//* edit assigned role 
