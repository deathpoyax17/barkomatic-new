...
if(isset($_POST["action"]) && $_POST["action"] == "all_passenger_data") {
    session_start();
    all_passenger_data($con);
}
...
$ship_comp = check_input($_POST['ship_comp']);
    $m_name = check_input($_POST['m_name']);
     $l_name = check_input($_POST['l_name']);
    ...
    $pass1 = check_input($_POST['rl_password']);
    $pass = check_input(sha1($pass1));
...
function create_ticket($con) {
    ...
    $timestamp = date("Y-m-d H:i:s");
        ...
        $stmt_insrt_sd->bind_param('sss', $tckt_qnty,$tckt_stats,$tckt_owner);
        $stmt_insrt_sd->execute();
        $stmt_insrt_sd->close();
